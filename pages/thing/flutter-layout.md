...
布局约束在布局中是很常见的概念，因为之前开发的都是浏览器端，在浏览器端布局约束是非常松的，所以你几乎感觉不到约束的存在，比如你可以在一个`div（100×100）`的中放一个`img（50×50）`而没有任何问题，而在flutter中，你可以试试在`Container（200×200）`在中放一个大小`FlutterLogo（100×100）`的
